...
import { FiMenu, FiX, FiChevronDown, FiUser, FiSearch, FiShoppingCart } from "react-icons/fi";
import gsap from "gsap";
import { Link } from "react-router-dom";

const navItems = [
    { label: "Products", submenu: ["New", "Popular", "Discounts"] },
    { label: "Company", submenu: ["About", "Careers", "Team"] },
    { label: "Support", submenu: ["Contact", "Help", "Shipping"] },
];
...
export default function Navbar() {
    ...
    // Handle submenu panel transition (desktop)
    useEffect(() => {
        if (submenuPanelRef.current) {
            if (activeDesktop !== null) {
                ...
                gsap.to(submenuPanelRef.current, {
                    ...
                    duration: 0.3,
                    ease: "power2.inOut",
                    onComplete: () => submenuPanelRef.current.style.display = "none",
                });
            }
        }
    }, [activeDesktop]);

    // Mobile accordion toggling
    const toggleAccordion = (index) => {
        const isOpen = openAccordions.includes(index);
        const updated = isOpen ? openAccordions.filter(i => i !== index) : [...openAccordions, index];
        ...
        <header className="w-[86%] mx-auto max-w-[1300px]">
            ...
            <div className="flex justify-between items-center  w-90% px-6 py-4">
                <Link to={"/"} className="font-bold text-xl ">
                    <img src="/tata-chemicals-white.svg" className="w-50" alt="" />
                </Link>
            ...
            >
                {/* Left: Button below logo */}
                <div className="flex flex-col items-start">
                    <button className="text-white rounded text-sm font-semibold hover:bg-gray-200 transition">
                    ...
                    {activeDesktop !== null && (
                        <div className="max-w-4xl ml-auto px-8 py-6 grid grid-cols-2 md:grid-cols-3 gap-6">
                            {navItems[activeDesktop].submenu.map((sub, idx) => (
                                <div key={idx} className="text-gray-200 hover:text-gray-400 text-sm">
                                    <Link to={"/"}>{sub}</Link>
                                </div>
                            ))}
                        </div>
                                ...
                                >
                                    <div className="pl-4 py-2 space-y-1 text-sm text-gray-600">
                                        {item.submenu.map((sub, j) => (
                                            <div key={j} className="py-1 hover:text-black">{sub}</div>
                                        ))}
                                    </div>
                                </div>
                            </div>
